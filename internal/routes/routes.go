...
func Setup(r *gin.Engine, db *gorm.DB, cfg config.Config) {
	...
	pinnedSentenceHandler := handlers.NewPinnedSentenceHandler(db)
	...
	protected.POST("/pinned-sentences", pinnedSentenceHandler.Set)
	protected.GET("/pinned-sentences", pinnedSentenceHandler.List)
